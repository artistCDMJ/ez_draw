...
import bpy, os, mathutils, collections
...
CYCLES_SHADERS = ( ('BSDF_DIFFUSE', "Diffuse", "Diffuse Shader"), \
...
########################
# -----------------------------------------------------------------------------
class EZ_DRAW_image_to_canvas(Operator, AddObjectHelper):
    """Create mesh plane(s) from image files with the appropiate aspect ratio"""
    bl_idname = "ez_draw.image_to_canvas"
    bl_label = "Import Image as Canvas"
    ...
    align = BoolProperty(name="Align Planes", \
                         default=True, description="Create Planes in a row")

    align_offset = FloatProperty(name="Offset", \
                                 min=0, soft_min=0, default=0.1, description="Space between Planes")

    force_reload = BoolProperty(name="Force Reload", \
                                default=False, \
